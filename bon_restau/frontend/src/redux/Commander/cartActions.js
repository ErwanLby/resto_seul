...
export const addToCart = (id, qty) => async (dispatch, getState) => {
    const { data } = await axios.get(`https://my-restaurant-proj.herokuapp.com/api/products/${id}`); //"http://localhost:5000/products/${id}"); ///(`/api/products/${id}`); //(`https://my-restaurant-proj.herokuapp.com/api/products/${id}`);
